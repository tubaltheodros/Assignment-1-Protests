...
num_attendees <-protest$Attendees
...
The mean is much higher than the median which tells us that the graph is skewed
to the right. The majority of the attendence is fairly low, but there were some
that had a high attendance that raised the mean to a high number
...
# Why or why not?
It does not surprise me. Last year there were many protests with George Floyd, 
Covid, and more that caused an uprise in protest. Washington is one of the most progressive 
states in the nation, so seeing many protests about injustice is not surprising.
...
count_on_date <- function(date) {
  sum_dates <- sum(str_detect(dates,date))
  return(paste("There were", sum_dates, "on", date))
...
count_in_attendees <- function(attendees) {
  attendees_sum <- str_detect(num_attendees)
}
